  ...
  try {
    ...
    if (Array.isArray(result.insured_members)) {
      result.insured_members.forEach((member: any) => {
        if (member.gender === '') delete member.gender;
      });
    }
...
export const policyController = {
  ...
  async deletePolicyTerm(req: Request, res: Response) {
    const userId = extractUserId(req);
    if (!userId) {
      res.status(401).json({ error: "Unauthorized" });
      return;
    ...
    try {
      ...
      const result = await policyService.deletePolicyTerm(req.params.id as string);
      res.status(200).json({
        success: true,
