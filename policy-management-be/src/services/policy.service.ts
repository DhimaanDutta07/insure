...
export async function calculateAndSetCommission(policyInput: any) {
  ...
  const tryMatch = (pred: (r: any) => boolean) => allRules.find(pred);
  if (isDeductOn) {
  ...
  if (!activeRule) {
    const pred = (r: any) => r.siCondition === 'ALL_SI' && !r.customSIThreshold && (isDeductOn ? r.deductibleStatus === true : (r.deductibleStatus === false || r.deductibleStatus === null));
    activeRule = tryMatch(pred);
  }

  // Step 3: null SI (no bucket, no custom) + deductible match
  if (!activeRule) {
    const pred = (r: any) => !r.siCondition && !r.customSIThreshold && (isDeductOn ? r.deductibleStatus === true : (r.deductibleStatus === false || r.deductibleStatus === null));
    activeRule = tryMatch(pred);
  }

  // Step 4: custom SI threshold + deductible match
  if (!activeRule) {
    activeRule = tryMatch(r =>
  ...
  if (!activeRule) {
    ...
    activeRule = await prisma.commissionRule.create({
      data: {
        policy_name_id: effectiveId,
        commissionPercent: 12,
        policyStatus: requestedStatus as any,
        deductibleType: 'ALL_SI' as any,
        ageCondition: 'LESS_THAN_60' as any,
        siCondition: hasSIClassification ? (siCondition as any) : null,
