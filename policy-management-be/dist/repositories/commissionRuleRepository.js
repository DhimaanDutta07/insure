...
exports.commissionRuleRepository = {
    ...
    getCommissionDashboardStats: async (timeRange) => {
        try {
            ...
            const totalCommission = await prismaClient_1.default.commissionJournal.aggregate({
                where: {
            ...
            const totalPoliciesWithCommission = await prismaClient_1.default.commissionJournal.count({
                where: {
                    calculatedAt: { gte: startDate },
                },
            });
            // Get commission by company from CommissionJournal joined with Policy
            const commissionByCompany = await prismaClient_1.default.commissionJournal.groupBy({
                by: ['policy_id'],
                where: {
                    calculatedAt: { gte: startDate },
                },
            ...
            const policyIds = commissionByCompany.map((c) => c.policy_id).filter((id) => id !== null);
            const policiesForCompany = await prismaClient_1.default.policy.findMany({
                where: { id: { in: policyIds } },
                select: { id: true, company_id: true },
            });
            const policyCompanyMap = new Map(policiesForCompany.map(p => [p.id, p.company_id]));
            const companyIds = Array.from(new Set(policiesForCompany.map(p => p.company_id).filter((id) => id !== null)));
            ...
            const companyCommissionMap = new Map();
            ...
            const commissionByPolicyName = await prismaClient_1.default.commissionJournal.groupBy({
                by: ['policy_id'],
                where: {
                    calculatedAt: { gte: startDate },
                },
            ...
            const policyIdsForName = commissionByPolicyName.map((p) => p.policy_id).filter((id) => id !== null);
            const policiesForName = await prismaClient_1.default.policy.findMany({
                where: { id: { in: policyIdsForName } },
                select: { id: true, policy_name_id: true },
            });
            const policyNameIds = Array.from(new Set(policiesForName.map((p) => p.policy_name_id).filter((id) => id !== null)));
            const policyNames = await prismaClient_1.default.policyName.findMany({
                where: { id: { in: policyNameIds } },
                select: { id: true, name: true },
            });
            const policyNameMap = new Map(policyNames.map((p) => [p.id, p.name]));
            // Aggregate commission by policy name
            const policyNameCommissionMap = new Map();
            for (const entry of commissionByPolicyName) {
                const policy = policiesForName.find((p) => p.id === entry.policy_id);
            ...
            const journalEntries = await prismaClient_1.default.commissionJournal.findMany({
                where: {
                    calculatedAt: { gte: startDate },
                },
                select: {
