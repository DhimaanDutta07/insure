...
exports.policyService = {
    ...
    async getAllPolicies(query = {}) {
        ...
        const [data, total] = await Promise.all([
            prismaClient_1.default.policy.findMany({
                ...
                skip,
                take,
                include: policyRepository_1.POLICY_LIST_INCLUDE,
            }),
            prismaClient_1.default.policy.count({ where }),
        ]);
        const result = {
            data,
            total,
    ...
    async deletePolicyTerm(id) {
        const result = await policyRepository_1.policyRepository.deletePolicyTerm(id);
        lruCache_1.policyListCache.delete(`policy:${id}`);
        lruCache_1.policyListCache.deleteByPrefix('policies:');
        lruCache_1.dashboardCache.deleteByPrefix('dashboard:');
        return result;
